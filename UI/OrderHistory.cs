...
namespace UI
{
    public class OrderHistory : IMenu
    {
        ...
        private void ViewCustOrderHistory()
        {
            ...
            {
                Console.WriteLine($"[{i}] {custs[i].Name} {custs[i].Id}");
            }
            ...
            Console.WriteLine($":: All Orders of {custToView.Name} ::");
            if(orders.Count == 0)
            {
            ...
            for(int i = 0; i < orders.Count; i++)
            {
                ...
                List<OrderItem> oi = _bl.GetOrderItems(orders[i].OrderId);
                for(int j = 0; j < oi.Count; j++)
                {
                    //Log to console the order quantity and the name of the Brew ordered.
                    Console.WriteLine($"{oi[j].OrderQuantity} {_bl.GetBrewById(oi[j].BrewId).ToString()}");
                }
            }
        }
    }
